...
如果没有 可视化与 Markdown 报告，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `可视化与 Markdown 报告` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
5. [3Sum - 三数之和](https://leetcode.com/problems/3sum/)
   ...
   - 解题步骤：先确定左右指针各自含义，再决定每一步移动哪一边，最后处理相遇或越界。
   - 易错点：最容易错的是指针移动条件、去重、边界字符和漏掉最后一次比较。
6. [Container With Most Water - 盛最多水的容器](https://leetcode.com/problems/container-with-most-water/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：双指针 / 贪心
   ...
   - 解题步骤：先确定左右指针各自含义，再决定每一步移动哪一边，最后处理相遇或越界。
   - 易错点：最容易错的是指针移动条件、去重、边界字符和漏掉最后一次比较。
7. [Design Add and Search Words Data Structure - 设计添加与搜索单词的数据结构](https://leetcode.com/problems/design-add-and-search-words-data-structure/)
   ...
   - 解题步骤：先明确要快速查询什么，再用 dict/set/Counter 或前缀树保存状态。
   - 易错点：最容易错的是先查再存还是先存再查，以及重复值、空输入和 key 的设计。
8. [Best Time to Buy and Sell Stock II - 买卖股票的最佳时机 II](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-ii/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：贪心 / Quant
   - 多解法：DP 持仓；累加正收益；峰谷法
   - 迁移：写清输入、状态变量、输出和失败边界。
...
# 2. 调用今天的核心能力
# 3. 检查输出并记录 Debug
print("run Day30 结果保存")
```

#### 5. 迁移说明

题目：写一段说明：今天能力如何迁移到 数据报告、策略复盘和作品展示。
...
- 关联题：[Merge Intervals](https://leetcode.com/problems/merge-intervals/)
- 难度：Medium
- 题型：排序 / 区间合并
- 为什么放在今天：报告和图表经常需要合并时间区间，适合引入区间题。
- 带注释解题提示：先按 start 排序，再维护当前区间；注释说明何时合并、何时开新区间。
...
今天的主题是 **可视化与 Markdown 报告**，不要只把它当成一个孤立知识点。它应该落到 `本地指标管道` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：字段含义、样本数量、缺失值、重复值、时间边界。
