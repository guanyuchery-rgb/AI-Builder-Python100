...
如果没有 FastAPI 最小服务，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
今天学的是：**FastAPI 最小服务**。它不只是一个孤立知识点，而是服务于「应用接口、自动化与交付」阶段的一个小能力：把脚本变成可重复运行的小工具、API 或页面。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `FastAPI 最小服务` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
1. [Container With Most Water - 盛最多水的容器](https://leetcode.com/problems/container-with-most-water/)
...
2. [Longest Substring Without Repeating Characters - 无重复字符的最长子串](https://leetcode.com/problems/longest-substring-without-repeating-characters/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：滑动窗口 / 哈希表
...
3. [Daily Temperatures - 每日温度](https://leetcode.com/problems/daily-temperatures/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：单调栈
...
4. [Evaluate Reverse Polish Notation - 逆波兰表达式求值](https://leetcode.com/problems/evaluate-reverse-polish-notation/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：栈 / 表达式解析
   - 多解法：递归解析；栈求值；操作函数字典
   - 迁移：写清输入、状态变量、输出和失败边界。
...
6. [Construct Binary Tree from Preorder and Inorder Traversal - 从前序与中序遍历序列构造二叉树](https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/)
   ...
   - 解题步骤：先判断空节点，再决定递归返回什么，最后合并左右子树结果。
   - 易错点：最容易错的是递归终止条件、返回值含义和只检查局部不检查整体。
7. [Permutations - 全排列](https://leetcode.com/problems/permutations/)
...
8. [Subsets - 子集](https://leetcode.com/problems/subsets/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：回溯 / 位运算
...
# 2. 调用今天的核心能力
# 3. 检查输出并记录 Debug
print("run Day39 结果保存")
```

#### 5. 迁移说明

题目：写一段说明：今天能力如何迁移到 本地服务和 Agent tool HTTP 化。
...
- 关联题：[Daily Temperatures](https://leetcode.com/problems/daily-temperatures/)
- 难度：Medium
- 题型：单调栈
- 为什么放在今天：FastAPI 之后不加框架难度，改练一个工业常见的等待时间问题。
- 带注释解题提示：栈里保存还没找到更高温的索引；注释说明新温度如何结算旧索引。
...
今天的主题是 **FastAPI 最小服务**，不要只把它当成一个孤立知识点。它应该落到 `DataTool 交付` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：输入参数、失败重试、运行命令、交付说明。
