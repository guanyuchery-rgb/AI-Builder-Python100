...
题目：写一段说明：今天能力如何迁移到 进入 Web/API/LLM/Quant 主题的共同地基。
...
- 难度：Medium
- 题型：滑动窗口 / 频次计数
- 为什么放在今天：阶段三 阶段复盘 后安排一题中等滑动窗口，训练窗口长度、频次和可替换次数之间的关系。
...
今天的主题是 **本地指标管道原理**，不要只把它当成一个孤立知识点。它应该落到 `本地指标管道` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：字段含义、样本数量、缺失值、重复值、时间边界。
