...
今天先不从定义开始，而是先看问题：**如何用「GitHub Actions 最小 CI」完成 `一个 Python CI workflow`？**
...
GitHub Actions 最小 CI 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「让 GitHub 自动运行测试、yaml workflow pytest、输入输出边界」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 GitHub Actions 最小 CI，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
**解决什么问题（Problem）**：GitHub Actions 最小 CI 的核心不是背概念，而是把本地脚本整理成别人也能运行、检查和维护的小工具。
...
今天学的是：**GitHub Actions 最小 CI**。它不只是一个孤立知识点，而是服务于「应用接口、自动化与交付」阶段的一个小能力：把脚本变成可重复运行的小工具、API 或页面。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `GitHub Actions 最小 CI` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
1. [Course Schedule - 课程表](https://leetcode.com/problems/course-schedule/)
...
2. [Rotting Oranges - 腐烂的橘子](https://leetcode.com/problems/rotting-oranges/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：BFS / 多源扩散
...
3. [Accounts Merge - 账户合并](https://leetcode.com/problems/accounts-merge/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：并查集 / 图
   ...
   - 解题步骤：先把输入建模成节点和边，再选择 DFS/BFS/并查集/最短路，并记录 visited 或距离。
   - 易错点：最容易错的是越界、重复访问、起点选择、环和层数/距离统计。
4. [Meeting Rooms II - 会议室 II](https://leetcode.com/problems/meeting-rooms-ii/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：堆 / 扫描线
   - 多解法：最小堆；开始结束数组；扫描线
   - 迁移：写清输入、状态变量、输出和失败边界。
...
5. [Jump Game - 跳跃游戏](https://leetcode.com/problems/jump-game/)
...
6. [Gas Station - 加油站](https://leetcode.com/problems/gas-station/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：贪心
...
# 2. 调用今天的核心能力
# 3. 检查输出并记录 Debug
print("run Day42 结果保存")
```

#### 5. 迁移说明

题目：写一段说明：今天能力如何迁移到 开源项目质量保护。
...
今天的主题是 **GitHub Actions 最小 CI**，不要只把它当成一个孤立知识点。它应该落到 `DataTool 交付` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：输入参数、失败重试、运行命令、交付说明。
