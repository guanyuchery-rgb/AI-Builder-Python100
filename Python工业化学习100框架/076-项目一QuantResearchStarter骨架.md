...
今天先不从定义开始，而是先看问题：**如何用「QuantResearchStarter 骨架」完成 `Quant 主题骨架`？**
...
QuantResearchStarter 骨架 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「理解Quant 主题目录、project skeleton、输入输出边界」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 QuantResearchStarter 骨架，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后练习里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
```python
# Day76 - Quant 主题-QuantResearchStarter 骨架
project = {
    "goal": "复盘一个可解释的小练习",
    "input": "虚拟样例数据",
...
**解决什么问题（Problem）**：QuantResearchStarter 骨架 的核心不是背概念，而是把前面的小能力沉淀成可解释、可运行、可讲清楚的主题资产。
...
今天学的是：**QuantResearchStarter 骨架**。它不只是一个孤立知识点，而是服务于「主题实战与能力清单」阶段的一个小能力：把前面的能力整理成可解释、可运行、可讲清楚的主题。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `QuantResearchStarter 骨架` 放进后续 IC 挑战里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到 IC”。
...
题目：写一段说明：今天能力如何迁移到 能力清单Quant 主题。
...
- 难度：Medium
- 题型：字符串 / 中心扩展
- 为什么放在今天：主题实战期第一天，用计数版回文承接 Day75。
...
今天的主题是 **QuantResearchStarter 骨架**，不要只把它当成一个孤立知识点。它应该落到 `AI Builder 能力清单` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：主题 说明文档、测试、演示脚本、复盘和作品索引。
