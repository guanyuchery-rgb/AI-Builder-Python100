...
今天只学习一个核心主题：**把 `类型标注、配置与输入边界` 学成可运行、可解释、可迁移的知识点**。
...
类型标注、配置与输入边界 是 Python 工业化学习中的一个小能力。
...
验收：终端能打印 JSON，且本地生成 `outputs/day019/`。
...
任务：写 3 句话说明 `类型标注、配置与输入边界` 会如何迁移到 Data、Quant、LLM 或 Agent。
