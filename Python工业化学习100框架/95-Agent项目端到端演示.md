...
- Agent Demo：它把前面学过的语法、数据和项目方法放进可维护结构。
...
今天的交付物是：run_demo()。它以后可以作为 Quant 数据模块、LLM 报告模块或 Agent tool 的一个稳定零件。
