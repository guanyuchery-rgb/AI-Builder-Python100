...
今天只学习一个核心主题：**把 `logging 与 Debug 记录` 学成可运行、可解释、可迁移的知识点**。
...
logging 与 Debug 记录 是 Python 工业化学习中的一个小能力。
...
验收：终端能打印 JSON，且本地生成 `outputs/day017/`。
...
任务：写 3 句话说明 `logging 与 Debug 记录` 会如何迁移到 Data、Quant、LLM 或 Agent。
