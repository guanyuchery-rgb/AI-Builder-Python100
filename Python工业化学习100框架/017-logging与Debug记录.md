...
如果没有 logging 与 Debug 记录，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后练习里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
今天学的是：**日志是程序运行时留下的证据，比临时 print 更适合长期主题。**
...
LLM/Agent 主题尤其需要日志，因为模型输出有不确定性，工具调用也可能失败。基础阶段先学会记录输入规模、关键步骤和错误原因，后面才能做更复杂的 trace 和 evaluation。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `logging 与 Debug 记录` 放进后续 IC 挑战里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到 IC”。
...
7. 用 3 行话说明它如何服务于 `主题调试、Agent 执行记录和线上排错`。
...
题目：写一段说明：今天能力如何迁移到 主题调试、Agent 执行记录和线上排错。
