...
4. [Task Scheduler - 任务调度器](https://leetcode.com/problems/task-scheduler/)
   ...
   - 解题步骤：先明确数据结构里保存什么，再规定入队/入栈和弹出的条件。
   - 易错点：最容易错的是空结构访问、保存值还是下标、过期判断和弹出顺序。
5. [Clone Graph - 克隆图](https://leetcode.com/problems/clone-graph/)
...
6. [Best Time to Buy and Sell Stock II - 买卖股票的最佳时机 II](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-ii/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：贪心 / Quant
   - 多解法：DP 持仓；累加正收益；峰谷法
   - 迁移：写清输入、状态变量、输出和失败边界。
   ...
   - 解题步骤：先找每一步能安全做的局部选择，再说明这个选择为什么不会破坏全局答案。
   - 易错点：最容易错的是只凭直觉贪心，没有证明局部选择的安全性。
7. [Pacific Atlantic Water Flow - 太平洋大西洋水流问题](https://leetcode.com/problems/pacific-atlantic-water-flow/)
...
8. [Word Ladder - 单词接龙](https://leetcode.com/problems/word-ladder/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：BFS / 最短路径
