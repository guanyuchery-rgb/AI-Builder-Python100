...
如果没有 pytest 与最小测试，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后练习里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
测试的核心不是 pytest 语法，而是“把预期写成代码”。当你写 `assert average([1, 2, 3]) == 2`，你是在告诉主题：这个函数在这个输入下必须返回 2。以后代码改动后，测试能自动检查这个预期是否仍然成立。
...
失败的测试是一种反馈，不是麻烦。它告诉你实际输出和预期输出不一致。调试时不要急着改代码，先确认是测试预期错了，还是函数逻辑错了。这个判断能力会影响后面所有工程主题。
...
Quant 指标、清洗函数、Agent tool 都需要测试，否则主题越改越不敢动。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `pytest 与最小测试` 放进后续 IC 挑战里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到 IC”。
