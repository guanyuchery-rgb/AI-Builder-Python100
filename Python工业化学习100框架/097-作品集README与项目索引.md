...
今天的目标不是背定义，而是产出：**portfolio 说明文档**。
...
今天先不从定义开始，而是先看问题：**如何用「能力清单 说明文档 与主题索引」完成 `portfolio 说明文档`？**
...
能力清单 说明文档 与主题索引 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「把四个主题整理成能力清单、portfolio、输入输出边界」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 能力清单 说明文档 与主题索引，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后练习里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
**解决什么问题（Problem）**：能力清单 说明文档 与主题索引 的核心不是背概念，而是把前面的小能力沉淀成可解释、可运行、可讲清楚的主题资产。
...
今天学的是：**能力清单 说明文档 与主题索引**。它不只是一个孤立知识点，而是服务于「主题实战与能力清单」阶段的一个小能力：把前面的能力整理成可解释、可运行、可讲清楚的主题。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `能力清单 说明文档 与主题索引` 放进后续 IC 挑战里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到 IC”。
...
2. 手打最小案例并运行，确认得到 `portfolio 说明文档`。
...
今天的主题是 **能力清单 说明文档 与主题索引**，不要只把它当成一个孤立知识点。它应该落到 `AI Builder 能力清单` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：主题 说明文档、测试、演示脚本、复盘和作品索引。
