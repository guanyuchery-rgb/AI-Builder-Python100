...
如果没有 可重试 Data Client 与失败处理，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `可重试 Data Client 与失败处理` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
1. [Linked List Cycle - 环形链表](https://leetcode.com/problems/linked-list-cycle/)
...
2. [Remove Nth Node From End of List - 删除链表的倒数第 N 个结点](https://leetcode.com/problems/remove-nth-node-from-end-of-list/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：链表 / 双指针
...
3. [3Sum - 三数之和](https://leetcode.com/problems/3sum/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：排序 / 双指针
   ...
   - 解题步骤：先确定左右指针各自含义，再决定每一步移动哪一边，最后处理相遇或越界。
   - 易错点：最容易错的是指针移动条件、去重、边界字符和漏掉最后一次比较。
4. [Container With Most Water - 盛最多水的容器](https://leetcode.com/problems/container-with-most-water/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：双指针 / 贪心
   ...
   - 解题步骤：先确定左右指针各自含义，再决定每一步移动哪一边，最后处理相遇或越界。
   - 易错点：最容易错的是指针移动条件、去重、边界字符和漏掉最后一次比较。
5. [Sliding Window Maximum - 滑动窗口最大值](https://leetcode.com/problems/sliding-window-maximum/)
...
6. [Search in Rotated Sorted Array - 搜索旋转排序数组](https://leetcode.com/problems/search-in-rotated-sorted-array/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：二分 / 分段有序
