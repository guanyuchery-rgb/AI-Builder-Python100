...
Embedding 是 LLM / RAG / Agent 的基础，不要只背“向量”这个词。

先看库之间的分工：

| 库 | 核心对象 | 输入 | 输出 | 用途 |
| --- | --- | --- | --- | --- |
| `numpy` | `array` | 数字列表 | 向量 / 矩阵 | 手写余弦相似度 |
| `sklearn.metrics` | similarity / distance 工具 | 向量 | 相似度分数 | 评估检索效果 |
| `torch` | `Tensor`, `Embedding` | token id / 数值 | Tensor 向量 | 深度学习模型底层 |
| `transformers` | tokenizer / model | 文本 | token / embedding / 生成文本 | Transformer 和 LLM 基础 |
...
transformers tokenizer/model
  ↓
RAG 检索和 Agent memory
```

今天只要求理解输入输出和最小向量计算。完整模型训练放到后续更高阶路径，不塞进当天。
