...
今天的目标不是背定义，而是产出：**一个有 README、src、tests、data 的项目骨架**。
...
今天先不从定义开始，而是先看问题：**如何用「项目骨架、README 与 Git 工作流」完成 `一个有 README、src、tests、data 的项目骨架`？**
...
项目骨架、README 与 Git 工作流 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「项目结构、README、git status」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 项目骨架、README 与 Git 工作流，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
Path("outputs/day21_result.txt").write_text(str(processed), encoding="utf-8")
print(processed)
```
...
**解决什么问题（Problem）**：项目骨架、README 与 Git 工作流 的核心不是背概念，而是把原始数据变成可以检查、清洗、计算、保存和复盘的数据资产。
...
```python
def build_day021_note(input_name, output_name, risk):
    return {
        "day": "Day21",
        "topic": "项目骨架、README 与 Git 工作流",
        "input": input_name,
        "output": output_name,
        "risk": risk,
...
今天学的是：**项目骨架、README 与 Git 工作流**。它不只是一个孤立知识点，而是服务于「数据分析与本地数据系统」阶段的一个小能力：把原始数据变成可以检查、保存、复盘的数据资产。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `项目骨架、README 与 Git 工作流` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
```python
def day021_engineering_note(input_data):
    """记录今天知识点在工程项目里的使用边界。"""
    return {
        "day": "Day21",
        "topic": "项目骨架、README 与 Git 工作流",
...
5. [Accounts Merge - 账户合并](https://leetcode.com/problems/accounts-merge/)
...
6. [Meeting Rooms II - 会议室 II](https://leetcode.com/problems/meeting-rooms-ii/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：堆 / 扫描线
   - 多解法：最小堆；开始结束数组；扫描线
   - 迁移：写清输入、状态变量、输出和失败边界。
...
7. [Container With Most Water - 盛最多水的容器](https://leetcode.com/problems/container-with-most-water/)
   ...
   - 解题步骤：先确定左右指针各自含义，再决定每一步移动哪一边，最后处理相遇或越界。
   - 易错点：最容易错的是指针移动条件、去重、边界字符和漏掉最后一次比较。
8. [Longest Substring Without Repeating Characters - 无重复字符的最长子串](https://leetcode.com/problems/longest-substring-without-repeating-characters/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：滑动窗口 / 哈希表
...
# 2. 调用今天的核心能力
# 3. 检查输出并记录 Debug
print("run Day21 结果保存")
```

#### 5. 迁移说明

题目：写一段说明：今天能力如何迁移到 GitHub 开源项目入口。
...
今天的主题是 **项目骨架、README 与 Git 工作流**，不要只把它当成一个孤立知识点。它应该落到 `本地指标管道` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：字段含义、样本数量、缺失值、重复值、时间边界。
