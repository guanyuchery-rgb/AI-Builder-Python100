...
今天先不从定义开始，而是先看问题：**如何用「项目三：AgentToolPrototype 骨架」完成 `项目三骨架`？**
...
项目三：AgentToolPrototype 骨架 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「搭建正式项目三目录、agent skeleton、输入输出边界」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 项目三：AgentToolPrototype 骨架，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
**解决什么问题（Problem）**：项目三：AgentToolPrototype 骨架 的核心不是背概念，而是把前面的小能力沉淀成可展示、可运行、可讲清楚的项目资产。
...
```python
def build_day086_note(input_name, output_name, risk):
    return {
        "day": "Day86",
        "topic": "项目三：AgentToolPrototype 骨架",
        "input": input_name,
        "output": output_name,
        "risk": risk,
...
今天学的是：**项目三：AgentToolPrototype 骨架**。它不只是一个孤立知识点，而是服务于「项目实战与作品集」阶段的一个小能力：把前面的能力整理成可展示、可运行、可讲清楚的项目。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `项目三：AgentToolPrototype 骨架` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
```python
def day086_engineering_note(input_data):
    """记录今天知识点在工程项目里的使用边界。"""
    return {
        "day": "Day86",
        "topic": "项目三：AgentToolPrototype 骨架",
...
2. [Course Schedule - 课程表](https://leetcode.com/problems/course-schedule/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：图 / 拓扑排序
   ...
   - 解题步骤：先把输入建模成节点和边，再选择 DFS/BFS/并查集/最短路，并记录 visited 或距离。
   - 易错点：最容易错的是越界、重复访问、起点选择、环和层数/距离统计。
3. [Rotting Oranges - 腐烂的橘子](https://leetcode.com/problems/rotting-oranges/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：BFS / 多源扩散
...
4. [Accounts Merge - 账户合并](https://leetcode.com/problems/accounts-merge/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：并查集 / 图
   ...
   - 解题步骤：先把输入建模成节点和边，再选择 DFS/BFS/并查集/最短路，并记录 visited 或距离。
   - 易错点：最容易错的是越界、重复访问、起点选择、环和层数/距离统计。
5. [Gas Station - 加油站](https://leetcode.com/problems/gas-station/)
...
6. [Maximum Product Subarray - 乘积最大子数组](https://leetcode.com/problems/maximum-product-subarray/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：动态规划 / 数组
...
7. [Word Search - 单词搜索](https://leetcode.com/problems/word-search/)
...
8. [Palindrome Partitioning - 分割回文串](https://leetcode.com/problems/palindrome-partitioning/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：回溯 / 字符串
...
今天的主题是 **项目三：AgentToolPrototype 骨架**，不要只把它当成一个孤立知识点。它应该落到 `AI Builder 作品集` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：项目 README、测试、演示脚本、复盘和作品索引。
