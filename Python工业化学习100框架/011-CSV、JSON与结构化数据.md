...
CSV/JSON 不是只会 `load` 和 `dump`。真实主题里还要注意：
...
这一节不是把前面没学完的语法硬塞回来，而是把 `CSV、JSON 与结构化数据` 放进后续 IC 挑战里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到 IC”。
