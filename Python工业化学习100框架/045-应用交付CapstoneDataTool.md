# Day45 - 应用交付 Capstone：Data Tool

> 阶段四：应用接口、自动化与交付

## 学习定位

把 CLI、API、页面、CI 组成最小交付。

今天的目标不是背定义，而是产出：**一个可展示的数据工具**。
...
今天先不从定义开始，而是先看问题：**如何用「应用交付 Capstone：Data Tool」完成 `一个可展示的数据工具`？**
...
应用交付 Capstone：Data Tool 是用 Python 解决工程任务时的一组小工具。最简定义：它帮助你把「把 CLI、API、页面、CI 组成最小交付、integration checklist、输入输出边界」放进可运行流程里。先把它当成解决问题的方法，不要先背术语。

### 2. 为什么存在（Why）

如果没有 应用交付 Capstone：Data Tool，代码通常会变成一次性脚本：逻辑散、结果难复查、报错难定位、以后项目里也难复用。它存在的意义是把真实任务拆成更清楚的步骤，让你知道哪里输入、哪里处理、哪里输出。
...
**解决什么问题（Problem）**：应用交付 Capstone：Data Tool 的核心不是背概念，而是把本地脚本整理成别人也能运行、检查和维护的小工具。
...
今天学的是：**应用交付 Capstone：Data Tool**。它不只是一个孤立知识点，而是服务于「应用接口、自动化与交付」阶段的一个小能力：把脚本变成可重复运行的小工具、API 或页面。
...
这一节不是把前面没学完的语法硬塞回来，而是把 `应用交付 Capstone：Data Tool` 放进后续真实项目里补厚。你现在已经进入 Day10 之后，学习重点要从“我知道这个语法”升级为“我知道它在工程里解决什么问题、会踩什么坑、怎么迁移到项目”。
...
1. [Group Anagrams - 字母异位词分组](https://leetcode.com/problems/group-anagrams/)
...
2. [Product of Array Except Self - 除自身以外数组的乘积](https://leetcode.com/problems/product-of-array-except-self/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：数组 / 前后缀
...
3. [Maximum Subarray - 最大子数组和](https://leetcode.com/problems/maximum-subarray/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：动态规划 / 贪心
...
4. [Merge Intervals - 合并区间](https://leetcode.com/problems/merge-intervals/)
   - 难度安排：必做（当前阶段主线，建议提交 AC）
   - 题型：排序 / 区间
...
5. [House Robber - 打家劫舍](https://leetcode.com/problems/house-robber/)
   ...
   - 解题步骤：先定义 dp 状态，再写 base case，最后写状态转移并用小样例手算一遍。
   - 易错点：最容易错的是 base case、数组长度、不可达状态和状态转移方向。
6. [House Robber II - 打家劫舍 II](https://leetcode.com/problems/house-robber-ii/)
   - 难度安排：选做（能做就提交，卡住先看解析）
   - 题型：动态规划 / 环形约束
   - 多解法：拆成两段；递归；滚动 DP
   - 迁移：写清输入、状态变量、输出和失败边界。
   ...
   - 解题步骤：先定义 dp 状态，再写 base case，最后写状态转移并用小样例手算一遍。
   - 易错点：最容易错的是 base case、数组长度、不可达状态和状态转移方向。
7. [LRU Cache - LRU 缓存](https://leetcode.com/problems/lru-cache/)
   ...
   - 解题步骤：先明确要快速查询什么，再用 dict/set/Counter 或前缀树保存状态。
   - 易错点：最容易错的是先查再存还是先存再查，以及重复值、空输入和 key 的设计。
8. [Lowest Common Ancestor of a Binary Tree - 二叉树的最近公共祖先](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-tree/)
   - 难度安排：挑战（先写思路，再决定是否提交）
   - 题型：树 / 递归
...
# 2. 调用今天的核心能力
# 3. 检查输出并记录 Debug
print("run Day45 结果保存")
```

#### 5. 迁移说明

题目：写一段说明：今天能力如何迁移到 进入 LLM/Agent/Quant 项目的应用层。
...
- 关联题：[Merge Two Sorted Lists](https://leetcode.com/problems/merge-two-sorted-lists/)
- 难度：Easy
- 题型：链表 / 合并
- 为什么放在今天：应用交付 Capstone 后，用有序合并理解数据流拼接。
- 带注释解题提示：dummy 节点简化头部处理；注释说明 tail 永远指向结果链表末尾。
...
今天的主题是 **应用交付 Capstone：Data Tool**，不要只把它当成一个孤立知识点。它应该落到 `DataTool 交付` 里的一个可复用能力：输入清楚、处理过程可检查、输出可以保存，失败时能解释原因。重点检查：输入参数、失败重试、运行命令、交付说明。
