...
学习定位：掌握 groupby、agg、pivot 的基础用法。
...
summary = df.groupby("symbol")["return_rate"].agg(["mean", "std", "count"])
...
按 symbol 聚合收益和风险，是最小研究报告。

## LLM / Agent 关联

Agent 需要结构化 summary，而不是整张原始表。
