...
分析 prompt_version 或 model_version 随时间的分数趋势，训练滚动均值和版本对比。
...
### 核心知识：datetime / rolling / version trend
...
| 处理 | datetime / rolling / version trend |
| 输出 | `outputs/reports/version_trend.csv` |
...
把 created_at 转成 datetime，按日期和版本聚合。
...
```python
import pandas as pd
df = pd.DataFrame({"date": pd.date_range("2026-01-01", periods=3), "score": [0.6,0.7,0.8]})
print(df["score"].rolling(2).mean().tolist())
```
...
- 生成 `outputs/reports/version_trend.csv` 或对应文档。
- 输出 7 日滚动均值或按版本的趋势表。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `时间序列与版本趋势`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
