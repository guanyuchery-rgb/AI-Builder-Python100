...
建立数据质量报告：缺失、重复、越界、枚举异常、schema 变化。
...
### 核心知识：data quality / validation / anomaly report
...
| 处理 | data quality / validation / anomaly report |
| 输出 | `outputs/reports/data_quality.md` |
...
```python
rules = {"score_range": "0 <= score <= 1", "latency_positive": "latency_ms > 0"}
print(rules)
```
...
- 生成 `outputs/reports/data_quality.md` 或对应文档。
- 把错误写成可行动建议，而不是只打印报错。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `数据质量与异常报告`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
