...
用 pandas 建立评估数据体检报告：shape、dtypes、missing、describe、唯一值。
...
### 核心知识：DataFrame / dtypes / describe / missing
...
| 处理 | DataFrame / dtypes / describe / missing |
| 输出 | `outputs/reports/data_profile.md` |
...
把 raw eval logs 读成 DataFrame，输出字段类型和缺失率。
...
```python
import pandas as pd
df = pd.DataFrame([{"model":"a","score":0.8},{"model":"b","score":0.6}])
...
- 生成 `outputs/reports/data_profile.md` 或对应文档。
- 识别 score、latency、cost 是否被读成数字。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `Pandas评估数据体检`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
