...
把项目变成别人能运行的 GitHub 作品：README、目录说明、数据样例、截图、命令。
...
### 核心知识：README / reproducibility / screenshot
...
| 处理 | README / reproducibility / screenshot |
| 输出 | `README.md` |
...
```python
sections = ["Purpose", "Data", "Run", "Outputs", "Screenshots", "Resume"]
print("README sections:", sections)
```
...
- 生成 `README.md` 或对应文档。
- 截图放到 docs/images 或 outputs/figures。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `README截图与运行说明`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
