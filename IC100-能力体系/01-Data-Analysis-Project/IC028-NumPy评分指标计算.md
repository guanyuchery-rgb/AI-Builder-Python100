...
用 NumPy 计算统计指标和阈值标记：均值、标准差、分位数、z-score、异常样本。
...
### 核心知识：numpy array / quantile / z-score
...
| 处理 | numpy array / quantile / z-score |
| 输出 | `outputs/reports/numpy_metrics.json` |
...
```python
import numpy as np
scores = np.array([0.9, 0.85, 0.4, 0.88])
print({"p10": float(np.quantile(scores, 0.1)), "mean": float(scores.mean())})
```
...
- 生成 `outputs/reports/numpy_metrics.json` 或对应文档。
- 标记低于 P10 或 z-score 异常的评估样本。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `NumPy评分指标计算`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
