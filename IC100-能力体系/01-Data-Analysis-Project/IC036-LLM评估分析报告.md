...
写一份像工作产出的分析报告：结论先行，证据跟上，限制说明明确。
...
### 核心知识：analysis report / executive summary / evidence
...
| 处理 | analysis report / executive summary / evidence |
| 输出 | `outputs/reports/final_report.md` |
...
```python
findings = [{"claim":"模型A更稳", "evidence":"pass_rate +4%", "risk":"样本量小"}]
print(findings)
```
...
- 生成 `outputs/reports/final_report.md` 或对应文档。
- 每个结论至少连接一个表或图。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `LLM评估分析报告`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
