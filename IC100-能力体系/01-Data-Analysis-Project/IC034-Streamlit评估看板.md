...
做最小可用 dashboard：模型筛选、任务筛选、指标卡、图表、低分样本表。
...
### 核心知识：Streamlit / filters / KPI / table
...
| 处理 | Streamlit / filters / KPI / table |
| 输出 | `app/streamlit_app.py` |
...
```python
widgets = ["model_filter", "task_filter", "score_card", "failure_table"]
print({"dashboard_widgets": widgets})
```
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `Streamlit评估看板`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
