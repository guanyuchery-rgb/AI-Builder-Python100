...
把脏字段变成稳定字段：score 转 float，latency_ms 转 int，cost_usd 转 float，label 统一枚举。
...
### 核心知识：to_numeric / fillna / enum normalization
...
| 处理 | to_numeric / fillna / enum normalization |
| 输出 | `data/processed/eval_clean.csv` |
...
print([normalize_label(x) for x in ["Correct", " partially correct "]])
```
...
- 生成 `data/processed/eval_clean.csv` 或对应文档。
- 输出 clean 数据和 cleaning report。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `评估结果清洗标准化`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
