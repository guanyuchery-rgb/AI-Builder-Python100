...
### 核心知识：bar chart / line chart / scatter / annotation
...
| 处理 | bar chart / line chart / scatter / annotation |
| 输出 | `outputs/figures/model_score.png` |
...
```python
data = {"gpt-a": 0.82, "gpt-b": 0.76}
print("chart_data", sorted(data.items(), key=lambda x: x[1], reverse=True))
```
...
- 生成 `outputs/figures/model_score.png` 或对应文档。
- 图片要有标题、坐标轴、单位和保存路径。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `Matplotlib评估可视化`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
