...
把 clean 数据写入 SQLite，建立可复查、可查询的本地仓库。
...
### 核心知识：sqlite3 / schema / insert / primary key
...
| 处理 | sqlite3 / schema / insert / primary key |
| 输出 | `data/warehouse/eval_analytics.db` |
...
conn.execute("create table eval_results(id text, score real)")
conn.execute("insert into eval_results values (?, ?)", ("e1", 0.8))
print(conn.execute("select count(*) from eval_results").fetchone()[0])
```
...
- 生成 `data/warehouse/eval_analytics.db` 或对应文档。
- 写入 clean CSV，并验证行数。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `SQLite评估结果入库`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
