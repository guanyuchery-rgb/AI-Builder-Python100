...
把前面散落步骤收束成一个 `python scripts/run_pipeline.py`，从 raw 到 report 一键生成。
...
### 核心知识：pipeline / argparse / logging / reproducibility
...
| 处理 | pipeline / argparse / logging / reproducibility |
| 输出 | `scripts/run_pipeline.py` |
...
```python
steps = ["inspect_raw", "clean", "metrics", "warehouse", "report"]
for step in steps:
    print("run", step)
```
...
- 生成 `scripts/run_pipeline.py` 或对应文档。
- 运行后检查 clean、db、reports、figures 是否生成。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `一键复现脚本`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
