...
把事实表和维表拆开：eval_results 是事实表，models/tasks/prompts 是维表。
...
### 核心知识：fact table / dimension table / join keys
...
| 处理 | fact table / dimension table / join keys |
| 输出 | `docs/data_model.md` |
...
设计 eval_id、model_id、task_id、prompt_id。
...
```python
models = {"m1": {"provider": "openai", "family": "gpt"}}
result = {"eval_id": "e1", "model_id": "m1", "score": 0.82}
print({**result, **models[result["model_id"]]})
```
...
- 生成 `docs/data_model.md` 或对应文档。
- 写明哪些字段不能重复，哪些字段可以随实验变化。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `维表与实验元数据设计`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
