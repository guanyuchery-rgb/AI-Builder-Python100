...
把 Data 项目从泛泛的数据分析改成 LLM Evaluation Analytics Platform，明确业务问题、用户、指标和最小数据形态。
...
定义平台要回答的 3 个问题：哪个模型更稳、哪类任务更容易失败、哪些错误值得人工复查。
...
```python
project_scope = {"users": ["LLM engineer", "AI product", "data analyst"], "questions": ["哪个模型综合分最高", "哪类任务失败最多", "哪些样本需要人工复查"]}
print(project_scope)
```
...
- 生成 `docs/problem_statement.md` 或对应文档。
- 写出项目 README 第一版和 metrics dictionary。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `评估平台问题定义`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
