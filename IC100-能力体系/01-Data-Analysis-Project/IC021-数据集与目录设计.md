...
设计本地数据目录和样例数据，不依赖真实 API，先用可公开、可复现的 eval logs 跑通项目。
...
### 核心知识：目录结构 / raw processed outputs / 数据契约
...
| 处理 | 目录结构 / raw processed outputs / 数据契约 |
| 输出 | `data/raw/eval_logs.jsonl` |
...
建立 raw、processed、warehouse、reports、figures、app 目录。
...
folders = ["data/raw", "data/processed", "outputs/reports", "outputs/figures", "src", "app"]
...
- 生成 `data/raw/eval_logs.jsonl` 或对应文档。
- 生成 20 行以内 toy eval logs，字段包含 model、task、score、latency、cost、judge_label。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `数据集与目录设计`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
