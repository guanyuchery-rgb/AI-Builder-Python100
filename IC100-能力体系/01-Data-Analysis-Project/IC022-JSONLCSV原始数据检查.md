# IC022 - JSONLCSV原始数据检查

> IC100 Data Analysis Project：LLM Evaluation Analytics Platform。

## 1. 今日定位

检查 JSONL/CSV 原始数据是否能稳定读取，先不清洗，先知道字段、行数、坏行和编码问题。
...
### 核心知识：JSONL / CSV / schema inspection
...
| 处理 | JSONL / CSV / schema inspection |
| 输出 | `outputs/reports/raw_inspection.md` |
...
写 raw inspection：行数、字段列表、空字段、坏 JSON 行。
...
```python
import json
rows = [json.loads(line) for line in ['{"id":"e1","score":0.8}', '{"id":"e2","score":0.6}']]
print({"rows": len(rows), "fields": sorted(rows[0])})
```
...
- 生成 `outputs/reports/raw_inspection.md` 或对应文档。
- 保留原始文件，不在 raw 层覆盖数据。
- 能说清今天输入、处理、输出。
- 能写出今天最容易出错的 2 个点。
- 有一次 GitHub commit。
...
> 我在 `LLM Evaluation Analytics Platform` 中完成 `JSONLCSV原始数据检查`，能把 LLM 评估日志转化为可复现的数据资产和可解释分析结果。
