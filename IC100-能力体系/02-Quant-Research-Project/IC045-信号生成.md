...
### A1. [robot-return-to-origin](https://leetcode.com/problems/robot-return-to-origin/)
...
### A2. [baseball-game](https://leetcode.com/problems/baseball-game/)

- 难度：Easy
- 类型：栈 / 模拟
- 为什么做：这题训练 `栈 / 模拟` 的基本面试表达，同时帮助你把 Python 数据结构用熟。
- 思路：用栈保存“还没被匹配或处理”的元素，遇到闭合条件再弹出。
- 解题步骤：
   1. 定义入栈对象。
   2. 遇到匹配条件时检查栈顶。
   3. 最后检查栈是否清空或剩余状态是否合理。
- 易错点：空栈时直接 pop 会报错，必须先判断。
- 做题记录要求：在 LeetCode 官方页面提交，通过后把通过截图或提交记录链接写进当天 `review.md`。
...
围绕 Mini Quant Research Framework，完成「信号生成」阶段，所有指标都要能复现并解释风险。
...
> 我在 `Quant Research Project` 中完成 `信号生成` 阶段，能把知识点转化为可运行代码、可复现输出和可解释复盘。
