...
### A1. [populating-next-right-pointers-in-each-node](https://leetcode.com/problems/populating-next-right-pointers-in-each-node/)
...
### A2. [longest-consecutive-sequence](https://leetcode.com/problems/longest-consecutive-sequence/)
...
围绕 RAG Knowledge Assistant，完成「结构化输出」阶段，保留文档、检索、回答和评估证据。
...
> 我在 `LLM Application Project` 中完成 `结构化输出` 阶段，能把知识点转化为可运行代码、可复现输出和可解释复盘。
